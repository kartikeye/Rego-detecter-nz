...
const visionTimeoutMs = Number(process.env.VISION_TIMEOUT_MS ?? 60000);
...
app.post("/api/scan", async (req, res) => {
  try {
    ...
    const classificationCandidates = lookupProvider.getClassificationCandidates();
    ...
    const visionResponse = await fetch(`${visionServiceUrl}/vision/scan`, {
      ...
      body: JSON.stringify({ frameDataUrl, classificationCandidates }),
